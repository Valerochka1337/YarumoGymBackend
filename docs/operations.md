...
Раздел «Диагностика» читает `GET /admin/api/ai-diagnostics` через обычную административную
cookie-сессию. Пользовательский bearer-токен доступа не даёт. Ответы не кэшируются.
Доступны состояние БД, агрегированные состояния очереди и до 200 последних запусков
планировщика за 24 часа: этапы, длительность, модель, HTTP-статус и фиксированные категории
ошибок. Неизвестные коды провайдера не выводятся. Промпты, исходные ответы и сообщения
исключений, аргументы инструментов, данные аккаунтов и секреты не сохраняются.

Журнал находится только в памяти текущего процесса: после перезапуска очищается, другие
экземпляры сервера не объединяются. Он не восстанавливает ошибки до развёртывания.
Чтобы разобрать ошибку, откройте раздел, повторите проблемное действие в приложении,
обновите список и сравните время и этап сбоя. Отчёт можно скопировать локально.
Просмотр не запускает AI, не изменяет очередь и не исполняет серверные команды.

Ручной GitHub workflow `Production diagnostics` остаётся резервным способом получить
состояние контейнеров и обезличенную сводку существующих логов без деплоя. Он не может
восстановить исключения, которые прежний обработчик не записал.
