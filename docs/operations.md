...
Перед первым browser-web deploy создать публичную A-запись `app.valerochkagym.tech` на тот
же адрес, что и `api.valerochkagym.tech`. `install-browser-web.sh` отказывается работать при
расхождении DNS, распаковывает release в отдельный каталог, атомарно переключает symlink,
при необходимости получает отдельный Certbot-сертификат и только после локальных HTTPS-smoke
проверок оставляет новый Nginx server block. При ошибке восстанавливаются предыдущие symlink
и конфигурация; выданный сертификат не удаляется и остаётся пригодным для повторной попытки.
